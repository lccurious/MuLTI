...
https://github.com/brendel-group/cl-ica/blob/master/losses.py
"""
...
class LpSimCLRLoss(CLLoss):
    ...
    def loss(self, z1, z2_con_z1, z3, z1_rec, z2_con_z1_rec, z3_rec):
        ...
        # [batch_size, batch_size, num_dim]
        z1_z3_dist = z1_rec.unsqueeze(0) - z3_rec.unsqueeze(1)
        # [batch_size, num_dim]
        z1_z2_dist = z1_rec - z2_con_z1_rec
        
        # BUG: spline triggers the deterministic errors
        if self.spline is not None:
            z1_z2_dist, z1_z2_log_abs_det = self.spline(z1_z2_dist.contiguous())

        if self.p < 1.0:
            # add small epsilon to make calculation of norm numerically more stable
            neg = torch.norm(
                torch.abs(z1_z3_dist + 1e-12),
                p=self.p,
                dim=-1,
            )
            pos = torch.norm(
                torch.abs(z1_z2_dist) + 1e-12, p=self.p, dim=-1
            )
        else:
            # TODO: verify this
            ...
            neg = torch.norm(
                z1_z3_dist, p=self.p, dim=-1
            )
            pos = torch.norm(z1_z2_dist, p=self.p, dim=-1)

        if self.pow:
            neg = neg.pow(self.p)
            pos = pos.pow(self.p)

        # sum up the distances along the temporal axis
        if len(neg.shape) == 3 and len(pos.shape) == 2:
            neg = torch.sum(neg, -1)
            pos = torch.sum(pos, -1)
        # all = torch.cat((neg, pos.unsqueeze(1)), dim=1)

        # apply the jacobian of componentwise noise transformation
        if self.spline is not None:
            pos += z1_z2_log_abs_det
...
class ConditionalLpSimCLRLoss(LpSimCLRLoss):
    def __init__(
        self,
        p: int,
        tau: float = 1.0,
    ...
    def loss(self, z1, z2_con_z1, z3, z1_rec, z2_con_z1_rec, z3_rec, z2_con_z1_ct):
        del z1, z2_con_z1, z3
        ...
        if self.p < 1.0:
            ...
            pos = torch.norm(
                torch.einsum("ijk,i->ijk", torch.abs(z1_rec - z2_con_z1_rec), z2_con_z1_ct) + 1e-12, p = self.p, dim=-1
            )
        else:
            neg = torch.norm(
                z1_rec.unsqueeze(1) - z3_rec.unsqueeze(0), p = self.p, dim=-1
            )
            pos = torch.norm(torch.einsum("ijk,i->ijk", (z1_rec - z2_con_z1_rec), z2_con_z1_ct), p=self.p, dim=-1)
    ...
    def __call__(self, z1, z2_con_z1, z3, z1_rec, z2_con_z1_rec, z3_rec, z2_con_z1_ct):
        return self.loss(z1, z2_con_z1, z3, z1_rec, z2_con_z1_rec, z3_rec, z2_con_z1_ct)
